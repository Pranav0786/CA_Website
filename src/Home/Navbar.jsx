...
const Navbar = () => {
  ...
  const goToDashboard = () => {
    navigate("/layout"); // adjust if you have role-based dashboards
  };
    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-black/80 backdrop-blur-md border-b border-blue-400/30 shadow-lg">
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center space-x-10 text-white font-medium text-lg">
          ...
          {/* ✅ Show profile if logged in, else show Sign In */}
          {profile ? (
            <li className="flex items-center gap-3">
              <span className="text-blue-300 font-semibold">{profile.name}</span>
              <button
                onClick={goToDashboard}
                className="px-5 py-2 rounded-full bg-green-500/20 border border-green-400 text-white font-semibold hover:bg-green-500/30 hover:shadow-[0_0_15px_rgba(0,255,100,0.6)] transition duration-300"
              >
                Go to Dashboard
              </button>
            </li>
